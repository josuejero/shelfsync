...
def upgrade() -> None:
    ...
    op.add_column(
        "shelf_items", sa.Column("goodreads_book_id", sa.String(length=40), nullable=True)
    )
    op.add_column(
        "shelf_items", sa.Column("normalized_title", sa.String(length=600), nullable=False)
    )
    op.add_column(
        "shelf_items", sa.Column("normalized_author", sa.String(length=400), nullable=False)
    )
    op.add_column(
        "shelf_items", sa.Column("normalized_key", sa.String(length=1100), nullable=False)
    )
    op.add_column("shelf_items", sa.Column("needs_fuzzy_match", sa.Boolean(), nullable=False))
    op.add_column(
        "shelf_items", sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False)
    )
    op.drop_index(op.f("ix_shelf_items_user_title_author"), table_name="shelf_items")
    op.create_index(
        "ix_shelf_items_user_normkey", "shelf_items", ["user_id", "normalized_key"], unique=False
    )
    ...
    op.add_column(
        "shelf_sources", sa.Column("last_imported_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.create_unique_constraint(
        "uq_shelf_source_user_type_ref", "shelf_sources", ["user_id", "source_type", "source_ref"]
    )
